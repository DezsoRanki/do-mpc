...
class PathFollowingEnv(gym.Env):
    ...
    **Note:** While the ranges above denote the possible values for observation space of each element,
    ...
    def step(self, action):
        ...
        self.beta = np.arctan((self.lr / (self.lf + self.lr)) * np.tan(self.delta_a))
        # dynamics
        self.model.set_rhs('theta', self._theta)
        self.model.set_rhs('x_path', self._theta)
        self.model.set_rhs('y_path', 2 * self.theta * self._theta)
        self.model.set_rhs('phi_path', (1 / (1 + 4 * self.theta * self.theta)) * 2 * self.theta * self._theta)
        ...
        self.observation = (x0[1][0], x0[2][0], x0[3][0], x0[4][0], x0[5][0], x0[6][0])
        ...
        if self.render_mode == "human":
            self.render()
        return np.array(self.observation, dtype=np.float32), reward, done, {}
    ...
    def reset(self):
        ...
        self.observation = np.array([self.state[1][0], self.state[2][0], self.state[3][0], self.state[4][0], self.state[5][0], self.state[6][0]])
        self.path_error = inf
        self.steps_beyond_terminated = None
